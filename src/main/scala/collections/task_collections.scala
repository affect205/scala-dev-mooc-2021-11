...
object task_collections {
  ...
  def capitalizeIgnoringASCII(text: List[String]): List[String] = {
    if (text.isEmpty) text
      ...
      .slice(1, text.length)
      .map(x => if (isASCIIString(x)) x.toUpperCase else x.toLowerCase)
  }
   ...
   * В тексте встречаются числа от 0 до 9 (прим. хотя в тестах есть число 10...)
   *
   * Реализуйте метод который цифровые значения в строке заменяет на числа: 1 -> one, 2 -> two
   *
   * HINT: Для всех возможных комбинаций чисел стоит использовать Map
   * **/
  val numberDict = Map("0" -> "zero", "1" -> "one", "2" -> "two", "3" -> "three", "4" -> "four", "5" -> "five",
    "6" -> "six", "7" -> "seven", "8" -> "eight", "9" -> "nine", "10" -> "ten", "11" -> "eleven", "12" -> "twelve" // Доколе?)
  )

  def numbersToNumericString(text: String): String = {
    text.split("\\s")
      .map(x => numberDict.getOrElse(x, x))
      .mkString(sep = " ")
  }
